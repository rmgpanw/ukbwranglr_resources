...
4. Open `_targets.R` in a text editor and (i) hash `plan(callr)` (ii) un-hash `plan(batchtools_sge, template = "sge.tmpl")` (near the top)
5. Activate the conda environment and run in a terminal `Rscript -e "tar_make_future(workers = 3)"`. *Note: run this in a detached `tmux` session that will continue running in the background if you close your `ssh` connection before the process has finished*
